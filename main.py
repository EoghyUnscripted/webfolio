...
from forms import RegisterForm, CommentForm, LoginForm, CreatePostForm, send_email
import os
...
def contact():
    ...
    if request.method == 'POST':
        ...
        if send_email(data['name'], data['subject'], data['email'], data['message']):
        ...
        # If failed
            ...
            # Message to display on contact screen
...
def add_new_post():
    ...
    return render_template("make-post.html", form=form, current_user=current_user, year=this_year)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, post=post, current_user=current_user, is_edit=True, year=this_year)
